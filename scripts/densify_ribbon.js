...
function parseRibbonFromTs(tsText) {
  const re =
    /\{\s*s_km:\s*([0-9]+(?:\.[0-9]+)?)\s*,\s*lat:\s*([0-9]+(?:\.[0-9]+)?)\s*,\s*lon:\s*([0-9]+(?:\.[0-9]+)?)\s*\}/g;
  const pts = [];
  let m;
...
function densifyBySkm(points, stepKm) {
  if (!Array.isArray(points) || points.length < 2) {
    return { out: points ?? [], nonMonotonicCount: 0, flatCount: 0 };
  }
  ...
  for (let i = 0; i < points.length - 1; i++) {
    ...
    if (ds < 0) {
      nonMonotonicCount++;
      // s_km recule => on n’interpole pas
      continue;
    }
    if (ds === 0) {
      flatCount++;
...
function formatTs(points, meta) {
  ...
  lines.push(
    `// Original points: ${meta.originalCount} | Dense points: ${meta.denseCount}`
  );
  if (meta.nonMonotonicCount > 0) {
    lines.push(
      `// ⚠️ Warning: non-monotonic s_km segments: ${meta.nonMonotonicCount} (no interpolation applied on those)`
    );
  }
  if (meta.flatCount > 0) {
...
function main() {
  ...
  if (!inPath || !outPath) {
    console.error(
      "Usage: node densify_ribbon.js <input.ts> <output.ts> <stepKm>"
    );
    process.exit(1);
  }
  ...
  if (r.nonMonotonicCount > 0) console.warn("[densify] WARNING non-monotonic segments:", r.nonMonotonicCount);
  if (r.flatCount > 0) console.warn("[densify] NOTE flat segments (ds=0):", r.flatCount);
}
